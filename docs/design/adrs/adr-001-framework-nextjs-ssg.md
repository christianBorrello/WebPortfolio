...
- React-based components (relevant to the owner's professional profile)
...
### Astro
Strong SSG framework with smaller JS output and content-focused architecture. Supports React components via islands. **Rejected because**: the owner's professional stack includes React/Next.js -- demonstrating proficiency with Next.js is directly relevant to the portfolio's purpose as an engineering showcase. Astro's island architecture adds conceptual complexity for component communication without clear benefit at this site's scale.
...
- **Mitigation**: Pin major version, update on the owner's schedule
